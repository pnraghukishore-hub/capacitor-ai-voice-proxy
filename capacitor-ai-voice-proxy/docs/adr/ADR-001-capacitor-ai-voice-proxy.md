...
The combination of techniques here is novel in practice but relies on existing primitives (Web Crypto, MediaRecorder, Cloudflare Workers, Google APIs). Each individual component is prior art. The **composition** — Firebase-verified edge proxy for authenticated AI voice in a Capacitor WebView — is defensible as a trade secret but unlikely to meet the non-obviousness bar for a software patent, particularly post-*Alice*. The better IP strategy is:

- **Open source the pattern** (community adoption, citations, EB1A evidence)
- **Trademark KashRA** (brand protection, already in progress via LLC naming)
- **Copyright the implementation** (automatic, no filing needed)
